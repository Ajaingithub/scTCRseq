...
TCR_AJ_VZV_gt_1_CT <- function(object, savedir, clus_col="seurat_clusters",
                            TCR_col, group_col, group_val, 
                            split_col="orig.ident", column_name, total_clusters, clus_fact){
  # library(taRifx)
  library(ggplot2)
  library(Seurat)
  library(dplyr)
  ...
  require_df_noNA_subset[,clus_col] <- factor(require_df_noNA_subset[,clus_col],levels = clus_fact)
  clusters = unique(require_df_noNA_subset[,clus_col])
  # n <- clusters[order(clusters)]
  # n <- c(0:(total_clusters-1))
  df <- data.frame(matrix(ncol = length(clus_fact), nrow = length(clus_fact)))
  colnames(df) <- clus_fact
  rownames(df) <- clus_fact
  ...
  object_TCR_cluster_gt_1 <- object_TCR_cluster[rowSums(object_TCR_cluster) > 1,]
  
  object_TCR_cluster_gt_1[object_TCR_cluster_gt_1 > 0] <- 1
  
  for (i in 1:length(df)) {
    for (j in 1:length(df)) {
      df[i,j] <- as.data.frame(table(object_TCR_cluster_gt_1[,i] + object_TCR_cluster_gt_1[,j] == 2)[2])[,1]
    }
  }
  ...
  hm$Cluster1 <- factor(hm$Cluster1,levels = clus_fact)
  hm$Cluster2 <- factor(hm$Cluster2,levels = clus_fact)
  ...
  p <- ggplot(hm, aes(x=Cluster1, y=Cluster2, fill=Clones)) +
    ...
    labs(title = paste(group_val, sep = " ")) + # using a title instead
  ...
  pdf(paste(savedir,"/","clonal_sharing/",group_val,"_from_",group_col,"_TCR_number_gt_1.pdf",sep=""), width = 6, height = 6)
  print(p)
  dev.off()
  
  write.table(df,paste(savedir,"/","clonal_sharing/",group_val,"_from_",group_col,"_TCR_number_gt_1.txt",sep=""), 
  ...
  object_TCR_cluster <- table(require_df_noNA_subset[,TCR_col], require_df_noNA_subset[,clus_col])
  object_TCR_cluster <- object_TCR_cluster[grep("^_$",rownames(object_TCR_cluster),invert=TRUE),]
  dim(object_TCR_cluster)
  object_TCR_cluster_gt_1 <- object_TCR_cluster[rowSums(object_TCR_cluster) > 1,]
  dim(object_TCR_cluster_gt_1)
  # object_TCR_cluster[object_TCR_cluster > 0] <- 1
  # test <- head(object_TCR_cluster,20) %>% tail(10) ## this is more interesting
  column_sum <- colSums(object_TCR_cluster_gt_1) %>% as.vector()
  vec <- c()
  
  for (i in 1:length(df)) {
    for (j in 1:length(df)) {
      req <- object_TCR_cluster_gt_1[object_TCR_cluster_gt_1[,i] + object_TCR_cluster_gt_1[,j] > 1,]
      if(length(req) > 0){
  ...
  dir.create(paste(savedir,"/","clonal_sharing",sep = ""),showWarnings = FALSE)
  pdf(paste(savedir,"/","clonal_sharing/",group_val,"_from_",group_col,"_cluster_TCR_only_proportion_TRSS_gt_1.pdf",sep=""), width = 6, height = 6)
  print(p)
  dev.off()
  
  write.table(df,paste(savedir,"/","clonal_sharing/",group_val,"_from_",group_col,"_TRSS_table_gt_1.txt",sep=""), quote = F, 

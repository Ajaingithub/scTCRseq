...
TCR_AJ_VZV_gt_1 <- function(object, savedir, clus_col="seurat_clusters",
                       TCR_col, group_col, group_val,
                       split_col="orig.ident", column_name, total_clusters){
  # library(taRifx)
  library(ggplot2)
  library(Seurat)
  library(dplyr)
  ...
  pdf(paste(savedir,"/","clonal_sharing/",group_val,"_from_",group_col,"_TCR_number_gt_1.pdf",sep=""), width = 15, height = 10)
  print(p)
  dev.off()
  ...
  dir.create(paste(savedir,"/","clonal_sharing",sep = ""),showWarnings = FALSE)
  pdf(paste(savedir,"/","clonal_sharing/",group_val,"_from_",group_col,"_cluster_TCR_only_proportion_TRSS_gt_1.pdf",sep=""), width = 15, height = 10)
  print(p)
  dev.off()
